...
type 'node_value t = Empty | Node of 'node_value * 'node_value t list

let rec ( ^* ) s n =
  match n with 0 -> "" | 1 -> s | n when n > 0 -> s ^ s ^* (n - 1) | _ -> s
...
let rec string_of_subtyping_derivation_tree ?(print_context = true) level
    (t : subtyping_node t) =
  match t with
  | Empty -> ""
  | Node (v, children) ->
      ...
      let subtype =
        Printf.sprintf
          "%s <: %s)\n%s"
          (Print.Style.string_of_raw_typ
             [ANSITerminal.cyan]
             (Grammar.show_typ v.s))
          (Print.Style.string_of_raw_typ
             [ANSITerminal.cyan]
             (Grammar.show_typ v.t))
          (String.concat
             "\n"
             (List.map
...
let rec string_of_typing_derivation_tree ?(print_context = true) level t =
  match t with
  | Empty -> ""
  | Node (v, children) ->
      let string_of_term_or_decl =
        match v.term with
      ...
      let typing =
        Printf.sprintf
          ...
          (Print.Style.string_of_raw_typ
             [ANSITerminal.blue]
             (Grammar.show_typ v.typ))
          (String.concat
             "\n"
             (List.map
                (string_of_typing_derivation_tree ~print_context (level + 1))
                children))
      in
      string_of_rule ^ string_of_context ^ typing

let print_subtyping_derivation_tree ?(print_context = true) tree =
  print_string (string_of_subtyping_derivation_tree ~print_context 0 tree)
