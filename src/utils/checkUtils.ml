let check_well_formedness context typ =
  if not (WellFormed.typ context typ) then
    raise (Error.NotWellFormed (context, typ))

let check_avoidance_problem rule context x s =
  if Grammar.occurs_typ x s then Error.raise_avoidance_problem rule context x s

let check_subtype context s t =
  let history, is_subtype = Subtype.subtype ~context s t in
  if not is_subtype then Error.raise_subtype history s t
