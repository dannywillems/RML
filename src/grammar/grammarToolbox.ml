...
let rec string_of_term_for_fresh_variable t = match t with
  ...
  | Grammar.TermAscription(term, typ) ->
     Printf.sprintf
       "ASC_%s"
       (string_of_term_for_fresh_variable term)
  ...
  | Grammar.TermRecursiveRecord(typ, (z, decl)) ->
     Printf.sprintf
       "REC_%s_%s"
       z
       (string_of_decl_for_fresh_variable decl)
...
and string_of_decl_for_fresh_variable decl = match decl with
  | Grammar.TermTypeDeclaration(x, _) ->
     Printf.sprintf
       "T-DECL_%s"
       x
