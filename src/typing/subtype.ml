...
type questions = Grammar.nominal_typ * Grammar.nominal_typ

let question_already_asked questions s t =
  let b =
    List.exists
...
(* As selection rules are very close, we abstract it with this function.
   ...
   for t <: x.label.
*)
let rec rule_sel rule history already_asked context (x, label) t =
  let type_of_x =
  ...
  | Some s ->
    let derivation_tree_subtype, is_subtype =
      match rule with
      | SEL_SUB -> subtype_internal history already_asked context s t
      | SUB_SEL -> subtype_internal history already_asked context t s
    in
...
(* The main subtyping algorithm. Rules are very close to rules given in official
   papers.
*)
and subtype_internal history already_asked context s t =
  if question_already_asked already_asked s t
  then (
    DerivationTree.create_subtyping_node
      ~rule:"ALREADY ASKED"
      ~is_true:true
    ...
    let already_asked = (s, t) :: already_asked in
    match (s, t) with
    (* TOP
       Γ ⊦ S <: ⊤
    *)
    ...
    *)
    | Grammar.TypeProjection(x, label_x), Grammar.TypeProjection(y, label_y) ->
      ...
      let node_sel_sub, is_subtype_sel_sub =
        rule_sel SEL_SUB history already_asked context (x, label_x) t
      in
      if is_subtype_sel_sub
      then (node_sel_sub, is_subtype_sel_sub)
      else (
        let node_sub_sel, is_subtype_sub_sel =
          rule_sel SUB_SEL history already_asked context (y, label_y) s
        in
    ...
    *)
    | Grammar.TypeDeclaration(tag1, s1, t1), Grammar.TypeDeclaration(tag2, s2, t2)
      ...
      let left_derivation_tree, left_is_subtype =
        subtype_internal history already_asked context s2 s1
      in
      let right_derivation_tree, right_is_subtype =
        subtype_internal history already_asked context t1 t2
      in
      DerivationTree.create_subtyping_node
        ~rule:"TYP <: TYP"
    ...
    *)
    | (Grammar.TypeFieldDeclaration(a, t), Grammar.TypeFieldDeclaration(b, u))
      ...
      let history_subtype, is_subtype =
        subtype_internal history already_asked context t u
      in
      DerivationTree.create_subtyping_node
        ~rule:"FLD <: FLD"
        ~is_true:is_subtype
    ...
    *)
    | (Grammar.TypeRecursive(z, rec_t), Grammar.TypeRecursive(z', rec_t')) ->
      ...
      let history_subtype, is_subtype =
        subtype_internal history already_asked context' fresh_rec_t fresh_rec_t'
      in
      DerivationTree.create_subtyping_node
        ~rule:"UN-REC"
        ~is_true:is_subtype
    ...
    *)
    | (Grammar.TypeRecursive(z, t'), t) ->
      ...
      *)
      ...
      let history_subtype, is_subtype =
        subtype_internal history already_asked context' fresh_t' t
      in
      DerivationTree.create_subtyping_node
        ~rule:"UN-REC-I <:"
        ~is_true:is_subtype
    ...
    *)
    | (s, Grammar.TypeRecursive(z, t')) ->
      ...
      *)
      ...
      let context' = ContextType.add fresh_z fresh_t' context in
      let history_subtype, is_subtype =
        subtype_internal history already_asked context' s fresh_t'
      in
      DerivationTree.create_subtyping_node
        ~rule:"UN-<: REC-I"
        ~is_true:is_subtype
    ...
    | (Grammar.TypeProjection(x, label), u') ->
      rule_sel SEL_SUB history already_asked context (x, label) u'
    (* <: SEL.
    ...
    *)
    ...
    | (l, Grammar.TypeProjection(x, label)) ->
      rule_sel SUB_SEL history already_asked context (x, label) l
    ...
    *)
    | (s, Grammar.TypeIntersection(t, u)) ->
      let left_history_subtype, left_is_subtype =
        subtype_internal history already_asked context s t
      in
      let right_history_subtype, right_is_subtype =
        subtype_internal history already_asked context s u
      in
      DerivationTree.create_subtyping_node
        ~rule:"AND <:"
        ~is_true:(left_is_subtype && right_is_subtype)
    ...
    *)
    | (Grammar.TypeIntersection(Grammar.TypeRecursive(z1, s1), Grammar.TypeRecursive(z2, s2)), t) ->
      ...
      let history, is_subtype =
        subtype_internal history already_asked context fresh_s t
      in
      DerivationTree.create_subtyping_node
        ~rule:"UN-AND <: REC-REC"
        ~is_true:is_subtype
    ...
    | (Grammar.TypeIntersection(s, u), t) ->
      ...
      then (
        ...
        let history_right, is_subtype_right =
          subtype_internal history already_asked context u t
        in
        DerivationTree.create_subtyping_node
          ~rule:"AND2<:"
