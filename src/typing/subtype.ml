...
let atom_not_in_context atom context typ =
  if ContextType.mem atom context
  then (
    Printf.printf "Type : %a\n" (Print.Pretty.nominal_typ ()) typ;
    Printf.printf "##########################\n";
    Printf.printf "Atome : %a\n" AlphaLib.Atom.print atom;
    Printf.printf "##########################\n";
    Printf.printf "Context : %a\n"
      ContextType.Pretty.print
      context;
    assert false
  )

type question = Grammar.nominal_typ * Grammar.nominal_typ

let already_asked questions s t =
  List.exists
    (fun (s', t') -> Grammar.equiv_typ s s' && Grammar.equiv_typ t t')
    questions
...
(* As selection rules are very close, we abstract it with this function.
   ...
   for t <: x.label.
*)
let rec rule_sel questions rule history context (x, label) t =
  let type_of_x =
  ...
  | Some s ->
    let derivation_tree_subtype, is_subtype =
      match rule with
      | SEL_SUB -> subtype_internal questions history context s t
      | SUB_SEL -> subtype_internal questions history context t s
    in
...
(* The main subtyping algorithm. Rules are very close to rules given in official
   papers.
*)
and subtype_internal questions history context s t =
  if already_asked questions s t
  then (
    DerivationTree.create_subtyping_node
      ~rule:"ASKED"
      ~is_true:true
    ...
    let questions = (s, t) :: questions in
    match (s, t) with
    (* TOP
       Γ ⊦ S <: ⊤
    *)
    ...
    *)
    | Grammar.TypeDeclaration(tag1, s1, t1), Grammar.TypeDeclaration(tag2, s2, t2)
      ...
      let left_derivation_tree, left_is_subtype =
        subtype_internal questions history context s2 s1
      in
      let right_derivation_tree, right_is_subtype =
        subtype_internal questions history context t1 t2
      in
      DerivationTree.create_subtyping_node
        ~rule:"TYP <: TYP"
    ...
    *)
    | (Grammar.TypeFieldDeclaration(a, t), Grammar.TypeFieldDeclaration(b, u))
      ...
      let history_subtype, is_subtype =
        subtype_internal questions history context t u
      in
      DerivationTree.create_subtyping_node
        ~rule:"FLD <: FLD"
        ~is_true:is_subtype
    ...
    *)
    | Grammar.TypeProjection(x, label_x), Grammar.TypeProjection(y, label_y) ->
      ...
      let node_sel_sub, is_subtype_sel_sub =
        rule_sel questions SEL_SUB history context (x, label_x) t
      in
      if is_subtype_sel_sub
      then (node_sel_sub, is_subtype_sel_sub)
      else (
        (* And <: SEL if SEL <: failed *)
        let node_sub_sel, is_subtype_sub_sel =
          rule_sel questions SUB_SEL history context (y, label_y) s
        in
    ...
    *)
    | (Grammar.TypeRecursive(z, rec_t), Grammar.TypeRecursive(z', rec_t')) ->
      ...
      let history_subtype, is_subtype =
        subtype_internal questions history context' fresh_rec_t fresh_rec_t'
      in
      DerivationTree.create_subtyping_node
        ~rule:"UN-REC"
        ~is_true:is_subtype
    ...
    *)
    | (Grammar.TypeRecursive(z, t'), t) ->
      ...
      *)
      atom_not_in_context z context s;
      let context' = ContextType.add z t' context in
      let history_subtype, is_subtype =
        subtype_internal questions history context' t' t
      in
      DerivationTree.create_subtyping_node
        ~rule:"UN-REC-I <:"
        ~is_true:is_subtype
    ...
    *)
    | (s, Grammar.TypeRecursive(z, t')) ->
      ...
      *)
      let context' = ContextType.add z t' context in
      let history_subtype, is_subtype =
        subtype_internal questions history context' s t'
      in
      DerivationTree.create_subtyping_node
        ~rule:"UN-<: REC-I"
        ~is_true:is_subtype
    ...
    *)
    | (s, Grammar.TypeIntersection(t, u)) ->
      ...
      let right_history_subtype, right_is_subtype =
        subtype_internal questions history context s u
      in
      DerivationTree.create_subtyping_node
        ~rule:"<: AND"
        ~is_true:(left_is_subtype && right_is_subtype)
    ...
    *)
    | (Grammar.TypeIntersection(Grammar.TypeRecursive(z1, s1), Grammar.TypeRecursive(z2, s2)), t) ->
      ...
      let history, is_subtype =
        subtype_internal questions history context fresh_s t
      in
      DerivationTree.create_subtyping_node
        ~rule:"UN-AND <: REC-REC"
        ~is_true:is_subtype
    ...
    *)
    | (Grammar.TypeProjection(x, label), u') ->
      rule_sel questions SEL_SUB history context (x, label) u'
    ...
    *)
    | (l, Grammar.TypeProjection(x, label)) ->
      rule_sel questions SUB_SEL history context (x, label) l
    (* AND-1-<:
       Γ ⊦ S <: T ∧ Γ ⊦ T ∧ U <: T
       =>
       Γ ⊦ S ∧ U <: T
    ...
    *)
    | (Grammar.TypeIntersection(s, u), t) ->
      ...
      then (
        ...
        let history_right, is_subtype_right =
          subtype_internal questions history context u t
        in
        DerivationTree.create_subtyping_node
          ~rule:"AND2<:"
    ...
    | _ ->
      Print.nominal_typ_endline s;
      Print.nominal_typ_endline t;
      print_endline "------------";
      DerivationTree.create_subtyping_node
        ~rule:"NO RULE"
        ~is_true:false
